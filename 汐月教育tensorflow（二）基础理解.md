...
summary分为多种函数，具体参考：<http://www.tensorfly.cn/tfdoc/api_docs/[Python](http://lib.csdn.net/base/python)/train.html#AUTOGENERATED-summary-operations>

## 2.

tf.merge_all_summaries( 合并汇总，似乎就是把当前的要输出的信息全都整合到一起 ) - >变成protobuf对象 - >[tf.train.Summarywriter](http://www.tensorfly.cn/tfdoc/api_docs/python/train.html#SummaryWriter)( <logdir磁盘目录>, <GraphDef> ) - > 将proto对象写入磁盘 
